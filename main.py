...
class MainWindow(QWidget):
    ...
    def _create_schedule_table(self):
        ...
        self.schedule_table.setHorizontalHeaderLabels(
            ["id", "day", "subject", "room_numb", "start_time", "week", "i"])
    ...
    def _update_schedule_table(self):
        ...
        for i, r in enumerate(records):
            ...
            joinButton.clicked.connect(lambda ch, num=i: self._edit_timetable(num))
            deleteButton.clicked.connect(lambda ch, num=i: self._delete_from_timetable(num))

        joinButton = QPushButton("Join")
        self.schedule_table.setCellWidget(len(records), 7, joinButton)
        joinButton.clicked.connect(lambda ch, num=len(records): self._add_to_timetable(num))
        self.schedule_table.resizeRowsToContents()

    def _add_to_timetable(self, num):
        row = list()
        for i in range(self.schedule_table.columnCount()):
            try:
                print(self.schedule_table.item(num, i).text())
                row.append(self.schedule_table.item(num, i).text())
            except:
                row.append(None)
        print(row, num)
        try:
            columns = ["id", "day", "subject", "room_numb", "start_time", "week", "i"]
            self.cursor.execute(
                f"INSERT INTO timetable({columns[0]}, {columns[1]}, {columns[2]}, {columns[3]}, {columns[4]}, {columns[5]}, {columns[6]}) "
    ...
    def _edit_timetable(self, num):
        ...
        print(row, num)
        try:
            columns = ["i", "day", "subject", "room_numb", "start_time", "week"]
            for i in range(1, 6):
    ...
    def _delete_from_timetable(self, num):
        row = list()
        for i in range(self.schedule_table.columnCount()):
            try:
                row.append(self.schedule_table.item(num, i).text())
    ...
    def _create_teachers_tab(self):
        ...
        self.update_teachers_button.clicked.connect(self._update_schedule)
    ...
    def _update_teachers_table(self):
        ...
        for i, r in enumerate(records):
            ...
            joinButton.clicked.connect(lambda ch, num=i: self._edit_teachersTable(num))
            deleteButton.clicked.connect(lambda ch, num=i: self._delete_from_teacherstable(num))

        joinButton = QPushButton("Join")
        self.teachers_table.setCellWidget(len(records), 3, joinButton)
        joinButton.clicked.connect(lambda ch, num=len(records): self._add_to_teacherstable(num))

        self.teachers_table.resizeRowsToContents()

    def _edit_teachersTable(self, num):
        row = list()
    ...
    def _delete_from_teacherstable(self, num):
        row = list()
        for i in range(self.teachers_table.columnCount()):
            try:
                row.append(self.teachers_table.item(num, i).text())
    ...
    def _add_to_teacherstable(self, num):
        row = list()
        for i in range(self.teachers_table.columnCount()):
            try:
                row.append(self.teachers_table.item(num, i).text())
    ...
    def _create_subjects_tab(self):
        ...
        self.update_subjects_button.clicked.connect(self._update_schedule)
    ...
    def _update_subjects_table(self):
        ...
        for i, r in enumerate(records):
            ...
            joinButton.clicked.connect(lambda ch, num=i: self._edit_subjectstable(num))
            deleteButton.clicked.connect(lambda ch, num=i: self._delete_from_subjectstable(num))

        joinButton = QPushButton("Join")
        self.subjects_table.setCellWidget(len(records), 2, joinButton)
        joinButton.clicked.connect(lambda ch, num=len(records): self._add_to_subjectstable(num))

        self.schedule_table.resizeRowsToContents()

    def _edit_subjectstable(self, num):
        row = list()
    ...
    def _delete_from_subjectstable(self, num):
        row = list()
        for i in range(self.subjects_table.columnCount()):
            try:
                row.append(self.subjects_table.item(num, i).text())
    ...
    def _add_to_subjectstable(self, num):
        row = list()
        for i in range(self.subjects_table.columnCount()):
            try:
                row.append(self.subjects_table.item(num, i).text())
